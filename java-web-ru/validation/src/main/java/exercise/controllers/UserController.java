...
public final class UserController {
    ...
    public static Handler createUser = ctx -> {
        ...
        Validator<String> emailValidator = ctx.formParamAsClass("email", String.class)
                .check(e -> EmailValidator.getInstance().isValid(e), "Некорректный email");

        Validator<String> passwordValidator = ctx.formParamAsClass("password", String.class)
                .check(p -> p.length() >= 4, "Пароль должен быть не короче 4-х символов")
        ...
        Map<String, List<ValidationError<? extends Object>>> errors =
                JavalinValidation.collectErrors(firstNameValidator, lastNameValidator,
